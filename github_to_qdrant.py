...
class MistralEmbeddingClient:
    ...
    def __init__(self, api_key: str, model: str = "codestral-embed", output_dimension: int = 1536):
        ...
        if not MISTRAL_AVAILABLE or Mistral is None:
            raise ImportError("Mistral AI library not available. Install with: pip install mistralai")
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            return [embedding.embedding for embedding in response.data if embedding.embedding is not None]
            
        except Exception as e:
            raise Exception(f"Mistral AI embedding error: {e}")
...
class GitHubToQdrantProcessor:
    ...
    def __init__(self, config_path: str):
        ...
        if branch:
            ...
            print(f"🌿 Target branch: default (main/master)")
        ...
        print(f"📝 Text splitter configured: {self.config['processing']['chunk_size']} chars/chunk with {self.config['processing']['chunk_overlap']} overlap")
